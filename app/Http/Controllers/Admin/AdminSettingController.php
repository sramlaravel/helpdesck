...
//dd($setting);
        ...
        try {
            ...
            // //Server settings
             ...
             $mail->addAddress($email,  setting('site_title'));
             $mail->addReplyTo(setting('mail_from_address'), setting('site_title'));
             // Content
             $mail->isHTML(true);
             $mail->Subject = 'Test Email';
             $mail->Body    = "This is a test email sent by system" ;

            // $mail->send() ;
         // $mail->send();

sendEmail($email ,'admin.email.template',[ 'emailBody' => "This is a test email sent by system" ]);

            Mail::to($email)->send( new Mailable(  'admin.email.template', [ 'emailBody' => "This is a test email sent by system" ]));
